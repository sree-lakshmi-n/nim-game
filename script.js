...
nimGame.prototype.getValue = function (key) {
  // To get the value of global variables
  return this[key];
};
nimGame.prototype.setValue = function (key, value) {
  // To set the value of global variables
  this[key] = value;
};
function $(selector) {
  // ID selector
...
function displayActivePlayer() {
  //result
  const currentPlayer =
    nim.getValue("activePlayer") === "Human" ? "Your" : "Computer's";
  $("active-player").innerText = `${currentPlayer} turn`;
}
...
function generateMatches() {
  // Matches generated. The game area is set.
  for (let rowNum = 1; rowNum <= nim.getValue("maxRowNum"); rowNum++) {
    $("game-area").innerHTML +=
      "<div id='row" +
      rowNum +
      "' class='row'><span class='row-label' onclick=humanMove(" +
      rowNum +
      ")>Row" +
      rowNum +
      "</span></div>";
...
function noOfMatchesLeft(rowNum) {
  ...
  for (let i = 1; i < $("row" + rowNum).children.length; i++) {
    if (!$("row" + rowNum).children[i].classList.contains("hide")) {
      // If the div's classList contains 'hide', then it isn't visible
      matchesLeft += 1;
    }
  }
  return matchesLeft;
...
function makeMove() {
  ...
  if (isAllZero(getStack())) {
    ...
    _("result-section")[0].classList.remove("hide");
    _("game-page")[0].classList.add("hide");
  }
...
function newGame() {
  ...
  enableBtn($("pc-move-btn"));
  enableRows();
  _("result-section")[0].classList.add("hide");
  _("game-page")[0].classList.remove("hide");
}

const startGame = () => {
  _("home-page")[0].classList.add("hide");
  _("game-page")[0].classList.remove("hide");
  newGame();
};
...
const homePage = () => {
  _("result-section")[0].classList.add("hide");
  _("home-page")[0].classList.remove("hide");
};

const toggleInstructions = (btn) => {
  if (Array.from(_("instructions")[0].classList).includes("hide")) {
    btn.innerText = "Hide Instructions";
  } else {
    btn.innerText = "See Instructions";
  }
  _("instructions")[0].classList.toggle("hide");
};
